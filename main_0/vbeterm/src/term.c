...
// all shits learned from internet. -> time to change.
static gboolean
...
#define CLR_GDK(x) (const GdkRGBA){.red = CLR_16(CLR_R(x)), .green = CLR_16(CLR_G(x)), .blue = CLR_16(CLR_B(x)), .alpha = 0}
	...
	gchar **command;
	gchar *command0 = NULL;
	command = cmd ? (gchar *[]){"/bin/sh", "-c", command0 = g_strdup(cmd), NULL} : (gchar *[]){command0 = g_strdup(g_application_command_line_getenv(cmdline, "SHELL")), NULL};
...
// the question is, how do you want to get it done?
// terminal is always a bad place.